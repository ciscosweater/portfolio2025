...
# Script para otimizar imagens com ALTA QUALIDADE
# Converte PNG para WebP com resolução mantida e compressão de alta qualidade

echo "Iniciando otimização de imagens com ALTA QUALIDADE..."
echo "================================================="

# Encontra todas as imagens acima de 500KB
find public/assets/screenshots/ -type f -name "*.png" -size +500k | while read img; do
    ...
    # Converte para WebP com ALTA QUALIDADE (90%) e mantém resolução original
    magick "$img" -quality 90 -define webp:lossless=false "$output"
...
original_total=$(find public/assets/screenshots/ -type f -name "*.png" -size +500k -exec du -k {} \; | awk '{sum += $1} END {print sum}')
